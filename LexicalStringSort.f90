...
! Author unkown
   ...
   public  :: StringSort
   private :: partition, quickStringSort, swap, stringComp, UpperCase
...
!  Subroutine StringSort uses the quickStringSort algorithm.
!  On input, StringArray is a one-dimensional array of character strings
!  to be StringSorted in ascending lexical order.
!  On output, StringArray is the StringSorted array.
!  If the optional argument CaseInsensitive is present and .true.,
!  the StringSort is case-insensitive.  If CaseInsensitive is absent or
!  if it is .false., the StringSort is case-sensitive.
...
!  for instance, this needs to be done before calling StringSort.



   subroutine StringSort (StringArrayIn, iStringSortIndexArray, CaseInsensitive)

     character (len = *), dimension (:), intent (in out)  :: StringArrayIn
     integer, dimension (:), intent (out)                 :: iStringSortIndexArray
...
! Local variables
     ...
     if (ios /= 0) then
       write(*, *) "Error allocating indexarray in LexicalStringSort::StringSort"
!      stop
     end if

     indexarray = (/ (k, k = low, high) /)

     call quickStringSort(StringArray, low, high)

     do i=low, high
     StringArrayIn(i) = StringArray(indexarray(i))
     iStringSortIndexArray(i) = indexarray(i)
     end do

     deallocate(indexarray, stat = ios)
     if (ios /= 0) then
       write(*, *) "Error deallocating indexarray in LexicalStringSort::StringSort"
!      stop   ! Fortran's Stop is not the best exit for Windows programming !
     end if

     return
   end subroutine StringSort


   recursive subroutine quickStringSort(StringArray, low, high)
     ...
     if (low < high) then
       ...
       call quickStringSort(StringArray, low, pivotlocation - 1)
       call quickStringSort(StringArray, pivotlocation + 1, high)
     end if
     return
   end subroutine quickStringSort
